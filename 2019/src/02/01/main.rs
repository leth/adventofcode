fn main() -> () {
    ...
    loop {
        ...
        match opcode {
            ...
            // Add
            {
                let lhsAddr = memory[position + 1];
                let rhsAddr = memory[position + 2];
                let destAddr = memory[position + 3];
                memory[destAddr] = memory[lhsAddr] + memory[rhsAddr];
            }
            2 =>
            // Add
            {
