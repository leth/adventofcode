...
fn find_intersection_points<'a>(
    lines_a: impl Iterator<Item = Line> + 'a + std::clone::Clone,
    lines_b: impl Iterator<Item = Line> + 'a + std::clone::Clone,
) -> impl Iterator<Item = Point> + 'a {
    use itertools::Itertools;
    lines_a
        .cartesian_product(lines_b)
        .filter_map(|(line_a, line_b)| find_intersection_point(line_a, line_b))
}
...
fn main() -> () {
    ...
    let lines_a = points_to_lines(pathstring_to_points(&path_string_a, &origin));
    let lines_b = points_to_lines(pathstring_to_points(&path_string_b, &origin));
