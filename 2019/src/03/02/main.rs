...
impl Line {
    ...
    fn len(&self) -> i32 {
        match self.orientation() {
            Orientation::Horizontal => (self.end.x - self.start.x).abs(),
            Orientation::Vertical => (self.end.y - self.start.y).abs(),
        }
    }

    fn rationalise(&self) -> Line {
...
fn pathstring_to_points<'a>(
    path: &'a str,
    start: &'a Point,
) -> impl Iterator<Item = Point> + 'a + std::clone::Clone {
    let start = start.to_owned();
    iter::once(start).chain(path.split(",").scan(
        start,
        |current: &mut Point, segment: &str| -> Option<Point> {
            let direction = segment.chars().nth(0).unwrap();
            let distance: i32 = segment[1..].parse().unwrap();
...
fn points_to_lines<'a>(
    mut points: impl Iterator<Item = Point> + 'a + std::clone::Clone,
) -> impl Iterator<Item = Line> + 'a + std::clone::Clone {
    let first = points.next().unwrap();
    points.scan(first, |last, next| {
        let line = Line {
            start: *last,
            end: next,
        };
...
#[derive(Debug, Copy, Clone)]
struct DistLine(i32, Line);

fn lines_to_distanced_lines<'a>(
    lines: impl Iterator<Item = Line> + 'a + std::clone::Clone,
) -> impl Iterator<Item = DistLine> + 'a + std::clone::Clone {
    lines.scan(0, |distance, line| {
        let current = *distance;
        *distance += line.len();
        Some(DistLine(current, line))
    })
}

fn find_intersection_point(line_a: Line, line_b: Line) -> Option<Point> {
    let line_a = line_a.rationalise();
    let line_b = line_b.rationalise();
    match (line_a.orientation(), line_b.orientation()) {
        ...
        (Orientation::Horizontal, Orientation::Vertical) => {
            if line_a.start.x < line_b.start.x
                && line_a.end.x > line_b.start.x
                && line_a.start.y > line_b.start.y
                && line_a.start.y < line_b.end.y
            {
                Some(Point {
                    x: line_b.start.x,
                    y: line_a.start.y,
        ...
        (Orientation::Vertical, Orientation::Horizontal) => {
            if line_a.start.y < line_b.start.y
                && line_a.end.y > line_b.start.y
                && line_a.start.x > line_b.start.x
                && line_a.start.x < line_b.end.x
            {
                Some(Point {
                    y: line_b.start.y,
                    x: line_a.start.x,
...
#[derive(Debug, Copy, Clone)]
struct IPoint(i32, i32, Point);

fn find_intersection_points<'a>(
    lines_a: impl Iterator<Item = DistLine> + 'a + std::clone::Clone,
    lines_b: impl Iterator<Item = DistLine> + 'a + std::clone::Clone,
) -> impl Iterator<Item = IPoint> + 'a {
    use itertools::Itertools;
    lines_a
        .cartesian_product(lines_b)
        .filter_map(|(dist_line_a, dist_line_b)| {
            match find_intersection_point(dist_line_a.1, dist_line_b.1) {
                Some(p) => Some(IPoint(
                    dist_line_a.0 + dist_line_a.1.start.distance(&p),
                    dist_line_b.0 + dist_line_b.1.start.distance(&p),
                    p,
                )),
                None => None,
            }
